...
const CrudResultados = () => {
    let emptyResultado = {
        idResultado: null,
        nombreResultado: "",
        descripcionResultado: "",
        nombreResponsableResultado: "",
        eje: {
            idEje: null,
        },
        indicador: {
    ...
    const [resultado, setResultado] = useState(emptyResultado);
    ...
    const saveResultado = () => {
        ...
        if (resultado.nombreResultado.trim()) {
            //let _resultados = [...resultados];
            let _resultado = { ...resultado };
            //UPDATE
            if (resultado.idResultado) {
                //const index = findIndexById(resultado.idResultado);
                resultadoService
    ...
    const deleteResultado = () => {
        const resultadoService = new ResultadoService();
        let _resultados = resultados.filter((val) => val.idResultado !== resultado.idResultado);
        resultadoService
    ...
    //         if (resultados[i].idResultado === id) {
    //             index = i;
    //             break;
    //         }
    //     }
            ...
            <div className="col-12">
                <div className="card">
                    ...
                    >
                        ...
                        <Column field="idResultado" header="ID" sortable></Column>
                        <Column field="nombreResultado" header="Resultado" sortable></Column>
                    ...
                    <Dialog visible={resultadoDlg} style={{ width: "450px" }} header="Detalle de resultado" modal className="p-fluid" footer={resultadoDlgFooter} onHide={hideDialog}>
                        ...
                        <div className="field">
                            <label htmlFor="nombreResultado">Nombre</label>
                            <InputText id="nombreResultado" value={resultado.nombreResultado} onChange={(e) => onInputChange(e, "nombreResultado")} required autoFocus className={classNames({ "p-invalid": submitted && !resultado.nombreResultado })} />
                            {submitted && !resultado.nombreResultado && <small className="p-invalid">Nombre es requerido.</small>}
                        </div>
                    </Dialog>
                    ...
                    <Dialog visible={deleteResultadoDlg} style={{ width: "450px" }} header="Confirm" modal footer={deleteResultadoDlgFooter} onHide={hideDeleteResultadoDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {resultado && (
                                <span>
                                    ¿Está seguro de eliminar el siguiente registro? <b>{resultado.nombreResultado}</b>?
                                </span>
                            )}
                        </div>
                    </Dialog>
...
export default React.memo(CrudResultados, comparisonFn);
